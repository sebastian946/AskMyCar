...
Optional: `ANTHROPIC_API_KEY` (enables Claude via `get_llm()`), `ANTHROPIC_MODEL` (default `claude-sonnet-5`), `OLLAMA_MODEL` (default `llama3.2`, used when `ANTHROPIC_API_KEY` is absent), `VOYAGE_API_KEY` (enables Voyage AI embeddings via `get_embeddings()`, `voyage-4-lite`; falls back to local Ollama when absent — needed for deployments where Ollama isn't reachable), `ALLOWED_ORIGINS` (comma-separated list, default covers the Vite dev server), `LANGSMITH_TRACING`/`LANGSMITH_API_KEY`/`LANGSMITH_ENDPOINT`/`LANGSMITH_PROJECT` (LangSmith tracing — works for both Anthropic and Ollama automatically via LangChain's callback system, no provider-specific code needed).
...
- `load_document.py`: `LoadManual(brand, model, year)` resolves the manual's presigned S3 URL via `config.bucket_actions.get_file_url`; if the manual isn't in S3 yet (`FileNotFoundError`), it transparently triggers `Web_Scraping(...).scrape_and_upload()` and retries. Loads the PDF (`PyPDFLoader`, handles S3/web URLs natively), splits it with `RecursiveCharacterTextSplitter`, and builds a Chroma vector store with `config.get_embeddings()` (Voyage AI if `VOYAGE_API_KEY` is set, otherwise local Ollama — independent of which chat LLM is selected). `.retriever()` is the main entrypoint other code should call. Note: a fresh in-memory Chroma store is built on every call (no persistence/caching across requests) — the whole manual gets re-embedded per request.
- `qa_chain.py`: `ManualQAChain(retriever)` wraps a retriever into a prompt | llm | parser chain. `ask(question)` is the main method — does retrieval once, returns `{"answer": str, "sources": list[str]}`. `text_prompt(question)` is a thin wrapper returning just the answer string (kept for `run_qa_test.py`). The LLM comes from `config.get_llm()`. This chain (`VectorStoreRetriever` → `ChatPromptTemplate` → LLM → `StrOutputParser`) is what LangSmith actually traces when `LANGSMITH_TRACING=true` — verified against a live project's run list. The embedding step in `load_document.py`'s `retriever()` runs *before* this chain exists, outside any active trace context, so it never shows up as its own LangSmith run regardless of whether Voyage or Ollama is used.
